...
init = {
    ...
    things: [Trigger "Foo", Item "Bar", Item "Baz"],
...
render = \model ->
    ...
    when model.state is
        ConfirmPage _ -> List.join [confirmScreen model]
        _ -> List.join [homeScreen model]
...
main =
    ...
    when model.state is
        DoSomething selected -> Stdout.line "Doing something with $(selected)... now exiting..."
...
mapSelected = \model ->
    x, idx <- List.mapWithIndex model.things
    when x is
        Trigger s ->
            row = 3 + (Num.toU16 idx)
